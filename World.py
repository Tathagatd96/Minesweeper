...
class World():
	...
	def run(self) -> int:
		""" Engine of the game """
		while (True):
			if type(self.__ai) == ManualAI or self.__debug or MyAI:
				self.__printWorld()
			...
			try: 
				...
				print("Error: Move is out of bounds!{}:{}:{}:{}".format(self.__perceptNumber,self.__ai.startX,self.__ai.startY,self.__ai.action_taken[:2]))
		...
		if self.__score == (self.__colDimension * self.__rowDimension) - self.__totalMines:
			if self.__rowDimension == 8 and self.__colDimension == 8:
				return 1,self.__movesMade,self.__lastAction,self.__lastTile
			elif self.__rowDimension == 16 and self.__colDimension == 16:
				return 2,self.__movesMade,self.__lastAction,self.__lastTile
			elif self.__rowDimension == 16 and self.__colDimension == 30:
				return 3,self.__movesMade,self.__lastAction,self.__lastTile
			else:
				return 1,self.__movesMade,self.__lastAction,self.__lastTile
		else:
			return 0,self.__movesMade,self.__lastAction,self.__lastTile
